...
import { Prescription } from "./TReceipts";
...
const MyReceipts = () => {
  const downloadPrescriptions = (prescriptions: any) => {
    ...
    prescriptions.forEach((prescription: any) => {
      ...
      doc.text(
        `Patient ID: ${prescription.doctorId} `,
        styles.marginLeft,
        yPos
      );
      ...
      prescription.name.forEach((i: any) => {
        doc.text(`Drug Name: ${prescription.name[i]}`, styles.marginLeft, yPos);
        yPos += 5;
      });

      prescription.items.forEach((i: any) => {
        doc.text(
          `Items: ${prescription.items[i]}`,
          styles.marginLeft,
          yPos + 10
        );
        yPos += 5;
      ...
      prescription.timeofday.forEach((i: any) => {
        doc.text(
          `Time of Day: ${prescription.timeofday[i]}`,
          styles.marginLeft,
          yPos + 15
        );
        yPos += 5;
      ...
      prescription.toBeTakenItems.forEach((i: any) => {
        doc.text(
          `To Be Taken Items: ${prescription.toBeTakenItems[i]}`,
          styles.marginLeft,
          yPos + 20
        );
        yPos += 25;
  ...
  const filterUniquePrescriptions = () => {
    const uniquePrescriptions = prescriptions.reduce<Prescription[]>(
      (acc, prescription) => {
        const existingPrescription = acc.find(
          (item: any) =>
            item.doctorId === prescription.doctorId &&
            item.createdAt === prescription.createdAt
        );
      ...
      <div className="container">
        <div className="prescriptions">
          <div className="d-flex  align-items-center justify-space-between mt-3 mb-3">
            <h1>Prescriptions</h1>
            <a
              href="https://wolt.com/en/aze/baku/stores?filters=primary%3Dpharmacy&sorting=recommended"
              className="d-flex align-items-center"
              style={{ textDecoration: "none", color: "black" }}
            >
              <span className="order-now-display">Order now!</span>{" "}
              <img src={woltLogo} alt="" style={{ width: "70px" }} />
            </a>
          </div>
          <div className="receipts-container">
            {filterUniquePrescriptions().map((item: any, index) => (
              <div key={index} className="reseipt-item">
                ...
                <div
                  onClick={() =>
                    handleDownloadClick(item.doctorId, item.createdAt)
                  }
