...
const Blogs = () => {
  ...
  useEffect(() => {
    dispatch(fetchPublishedBlogs()).then(() => setLoading(false));
    dispatch(fetchCategories());
    dispatch(getMe());
    dispatch(fetchUsers());
    if (!notificationShown) {
      toast.info(
        "Welcome dear Doctor! If you do not find your blogs here, please make sure  the status of blog is PUBLISH, or you complited your data from My Profile",
        {
          autoClose: 10000,
          position: "top-right",
        }
  ...
  const author =
    ...
    users?.find(
      (user) =>
        Array.isArray(blogs) &&
        blogs.find(
          (blog) => blog?.status === "publish" && blog?.authorId === user._id
        )
    );
        ...
        <>
          ...
          <div className="container d-flex align-item-center justify-center">
            <div className="blogs-list row d-flex align-item-center justify-center mb-4">
              <div className="find-blog  d-flex align-item-center justify-center">
                {" "}
                <Search
