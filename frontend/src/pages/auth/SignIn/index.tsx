...
const SignIn = () => {
  ...
  useEffect(() => {
    const isAnyUserActive =
      Array.isArray(content) && content.some((details) => details.status === 'Active');

    if (result && result.token && result.status === 'succeeded' && isAnyUserActive) {
    ...
    <>
      ...
      <div className="form-container sign-in-container">
        <form className="custom-form" onSubmit={handleSubmit(onSubmit)}>
          <h1>Sign In</h1>
          {errorMessage && <h3 className="error-message mt-2">{errorMessage}</h3>}
          ...
          <div className="form-group" style={{ position: 'relative' }}>
            ...
            >
              {showPassword ? (
                <i className="fa fa-eye-slash" aria-hidden="true" style={{ color: 'var(--primary-color)' }}></i>
              ) : (
                <i className="fa fa-eye" aria-hidden="true" style={{ color: 'var(--primary-color)' }}></i>
              )}
            </span>
