    ...
    return (
        <>
            <div className="container ">
                ...
                <div className="mt-3 d-flex align-items-center justify-space-between" style={{ width: '150px' }}><Link to='/blogs' style={{ textDecoration: 'none', color: 'var(--dark-text)' }}><i className="fa-solid fa-chevron-left" style={{ fontSize: '20px' }}></i> Back to Blogs</Link></div>
                        ...
                        <>                <div className="row mt-5 mb-5">
                            <div className="blog-container col-10">
                                ...
                                <div>
                                    ...
                                    {blog && (
                                        (() => {
                                            ...
                                            const binary = bufferr?.reduce((acc: string, byte: number) => acc + String.fromCharCode(byte), '');
                                            const base64Image = btoa(binary);
                                            return (
                                                <div className="d-flex justify-space-between author-info align-items-center" >
                                                    <div style={{ width: '220px', height: '100px' }} className="blog-author">
                                                        <div className="author-img"> <Link to={`/doctordetails/${blog?.authorId}`} >  <img src={`data:image/jpeg;base64, ${base64Image}`} alt="doctor-photo" /></Link></div>
                                                        <div className="d-flex flex-column ">
                                                            <h3>{username}</h3>
                                                            <span className="review-date mt-1"><i>{formattedDate(blog?.createdAt)}</i></span>
                                                        </div>
                                                    </div>
                                                    <div className="d-flex  align-items-center">
                                                        <img src={starIcon} alt="" /> <span style={{ fontSize: '20px' }}>(0)</span>
                                                    </div>
                                                </div>
                                            )
                                        })()
                                ...
                                <div className="blog-image mt-4">
                                    {blog?.image && blog?.image.data && Array.isArray(blog?.image.data) && (
                                        (() => {
                                            const bufferr = blog?.image.data;
                                            const binary = bufferr?.reduce((acc: string, byte: number) => acc + String.fromCharCode(byte), '');
                                            const base64Image = btoa(binary);
                                            return <Image src={`data:image/jpeg;base64, ${base64Image}`} alt="blog-image" style={{ width: "90%" }} preview={false} />;
                                        })()
                                    )}
                                </div>

                                <p className="desc mt-3 ml-2"><i>{blog?.description}</i></p>
                            ...
                            <div className="long-details">
                                <div className="mt-5 border pb-2">
                                    <span className={`${tab === 'feedback' && 'border-b'} py-2 px-2 feedback-btn `} onClick={() => setTab('feedback')}>Feedback</span>
                                    <span className={`${tab === 'blogs' && 'border-b'} py-2 px-2 feedback-btn `} onClick={() => setTab('blogs')}>Related Blogs</span>
                                </div>

                                <div className="feedback-details mt-5 pb-4">
                                    {tab === 'feedback' && <DoctorFeedback id={id} />}
                                    {tab === 'blogs' && <DoctorBlogs blogId={blog?._id} tagss={blog?.tags} />}
                                </div>
                            </div>
