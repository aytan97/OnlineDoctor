    ...
    return (
        <>
            <div className="container">
                ...
                <div className="mt-3 d-flex align-items-center justify-space-between" style={{ width: '150px' }}><Link to='/talkToDoctor' style={{ textDecoration: 'none', color: 'var(--dark-text)' }}><i className="fa-solid fa-chevron-left" style={{ fontSize: '20px' }}></i> <span>Back Doctors List</span></Link></div>
                                ...
                                <div className=" col-lg-8 ">
                                    <div className=" short-details ">
                                        <div className="doctor-image">
                                            {list && list?.image && list?.image.data && Array.isArray(list?.image.data) && (
                                                (() => {
                                                    ...
                                                    return <Image src={`data:image/jpeg;base64, ${base64Image}`} alt="blog-image" style={{ width: "100%" }} />;
                                                })()
                                            )}
                                        </div>
                                        ...
                                        <div className="short-info">
                                            <div className="speciality mb-2">
                                                {list && Array.isArray(list) && list?.categories?.map((catId: string, index: number) => {
                                            ...
                                            <div className="d-flex align-items-center mt-1">
                                                <span className="stars">
                                                    <img src={starIcon} alt="stars" /> <span style={{ fontWeight: 'bold' }}>{list?.avgRating || '0'}</span>
                                                </span>
                                                <span className="rating" >({list?.totalRating || '0'})</span>
                                            </div>
                                            <p className="hospital mt-2">{list?.currentWorkHospital}</p>
                                        </div>
                                    ...
                                    <div className="long-details">
                                        <div className="mt-5 border pb-2">
                                            <span className={`${tab === 'about' && 'border-b'} py-2 px-2 about-btn`} onClick={() => setTab('about')}>About</span>
                                            <span className={`${tab === 'feedback' && 'border-b'} py-2 px-2 feedback-btn `} onClick={() => setTab('feedback')}>Feedback</span>
                                            <span className={`${tab === 'blogs' && 'border-b'} py-2 px-2 feedback-btn `} onClick={() => setTab('blogs')}>Blogs</span>
                                        </div>
