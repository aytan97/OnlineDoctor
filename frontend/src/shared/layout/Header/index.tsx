    ...
    <div className='header-items ' ref={dropdownRef}>
      ...
      <header className='header d-flex justify-space-around'>
        <div className='nav-logo col-md-4 col-lg-6 col-xl-7 col-xxl-7 col-xs-6 col-sm-3'>
          <Link to='/' ><img src={logo2} className="img-fluid " /></Link></div>
        <div className="nav col-md-8 col-lg-6 col-xl-5 col-xxl-5 col-xs-6 col-sm-9  d-flex justify-flex-end ">
            ...
            <div >
              {isSmallScreen ? (
                <div className="dropdown">
                  <div className="dropdown-toggle" id="dropdownMenu2" data-bs-toggle="dropdown" aria-expanded="false"><img src={menuIcon} alt="menu" /></div>

                  <div className="dropdown-menu">
                    <div className='d-flex flex-column '>
                      <Link to='/' className='pb-2 dropdown-item nav-item text-decoration ' >Home</Link>
                      <Link to={`${result?.token ? '/talkToDoctor' : "/login"}`} className='pb-2 dropdown-item nav-item text-decoration'>Talk to Doctor</Link>
                      <Link to='/faq' className='pb-2 dropdown-item nav-item text-decoration'>FAQ</Link>
                      <Link to={`${result?.token ? '/blogs' : "/login"}`} className='pb-2 dropdown-item nav-item text-decoration'>Blogs</Link>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="navbar-nav">
                  <Link to='/' className='nav-item text-decoration pr-2'>Home</Link>
                  <Link to={`${result?.token ? '/talkToDoctor' : "/login"}`} className='nav-item text-decoration  pr-2'>Talk to Doctor</Link>
                  <Link to='/faq' className='nav-item text-decoration  pr-2'>FAQ</Link>
                  <Link to={`${result?.token ? '/blogs' : "/login"}`} className='nav-item text-decoration  pr-2'>Blogs</Link>
                </div>
              )}
            </div>
          </div>
          ...
          <div className='nav-utils '>
            ...
            {result?.token ? (<div className="App">
              <div className="content">
                <div className='profile d-flex justify-center align-items-center' onClick={handleHeaderUserModal}>
                  <span style={{ fontSize: '14px' }}>
                    {result?.token && user && `${user.data.firstname.substr(0, 1)} ${user.data.lastname.substr(0, 1)}`}
                  </span></div>
              </div>
            ...
            </div>) : (<Link to='/login' className='text-decoration' >Sign in</Link>)}
