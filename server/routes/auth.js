...
let base64ImageDefault = ''
try {
  const imagePath = path.join(__dirname, '../public/avatar.png')
  const imageBuffer = fs.readFileSync(imagePath)
  base64ImageDefault = `data:image/jpeg;base64,${imageBuffer.toString(
    'base64'
  )}`
} catch (err) {
  console.error('Failed to load default avatar:', err.message)
}
...
router.post('/', userValidationRules(), validateUser, async (req, res) => {
  try {
    ...
    const userImage =
      image && /^data:image\/\w+;base64,/.test(image)
        ? Buffer.from(image.split(';base64,').pop(), 'base64')
        : Buffer.from(
            base64ImageDefault.split(';base64,').pop() || '',
            'base64'
          )
...
router.post('/verify-otp', async (req, res) => {
  try {
    ...
    if (!userDetails) {
      return res
        .status(404)
        .json({ success: false, message: 'User not found in pending list' })
    }

    const user = await CommonUser.create({
      ...userDetails,
      status: userDetails.role === 'doctor' ? 'Waiting' : 'Active',
    })
    ...
    sendMail(
      ...
      'Welcome to Doctor Online',
      null,
      template.getTemplate(user.role === 'doctor' ? 'welcomeDoctor' : 'welcome')
    )
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Server error during OTP verification' })
  }
})
...
router.get('/getMe', async (req, res) => {
  try {
    ...
    const user = await CommonUser.findById(decoded.userId)
